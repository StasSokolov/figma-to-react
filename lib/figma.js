...
`;
  ...
  const visitNode = (node, parent, lastVertical, indent) => {
    ...
    if (['FRAME', 'RECTANGLE', 'INSTANCE', 'COMPONENT'].indexOf(node.type) >= 0) {
      ...
      if (node.characters.includes('BUTTON')) {
        content = [`<button key="${node.id}" placeholder="${node.characters}" value="${node.name.substring(7)}" />`]
      }
    ...
    const elem = node.characters === 'BUTTON' ? 'button' : 'div';

    function printDiv(styles, outerStyle, indent) {
      print(`<${elem} ${elem === 'button' ? '' : `style={${JSON.stringify(outerStyle)}}`} className="${elem === 'button' ? '' : outerClass}">`, indent);
      print(`  <div`, indent);
